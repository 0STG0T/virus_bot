...
**Активация звезд (activate_all_stars)** - КРИТИЧЕСКИ ВАЖНАЯ ЛОГИКА:
- **КРИТИЧНО**: Проверяет баланс ПЕРЕД активацией. Если баланс >= 100⭐ - НЕ активирует НИЧЕГО!
- **Причина**: Если баланс >= 100⭐, то звезды перестают падать с рулетки вообще (game mechanics)
- Только если баланс < 100⭐ - начинается процесс активации из инвентаря
...
- **Важно**: Активирует МИНИМУМ звезд, чтобы в инвентаре осталось >= 100⭐
- Если инвентарь < 100 звезд - не активирует ничего
- Если инвентарь >= 100 звезд - активирует минимум звезд, оставляя >= 100⭐ в инвентаре
- Сортирует по возрастанию (активирует сначала мелкие пачки для точного контроля остатка)
- Использует get_balance(use_cache=False) для точности проверки баланса

**Автопродажа подарков (auto_exchange_cheap_gifts)** - НОВАЯ ЛОГИКА:
- **КРИТИЧНО**: Продает ТОЛЬКО если в инвентаре > 100 неактивированных звезд (status='NONE')
- **Причина**: Нужно сохранять >= 100⭐ в инвентаре для лучшей вероятности ценных подарков
- Сканирует весь инвентарь сначала для подсчета неактивированных звезд
- Если неактивированных звезд <= 100⭐ - автопродажа НЕ запускается
- Если неактивированных звезд > 100⭐ - начинается продажа дешевых подарков
...
- Все подарки из фри спинов выводятся отдельным уведомлением в формате: "🎁 ФРИ СПИН | сессия | название | ценность⭐"
...
- Автоматические платные спины: "🎁 АВТО ПЛАТНЫЙ СПИН | сессия | название | ценность⭐"
